...
public class Security_password extends Fragment {

    com.rengwuxian.materialedittext.MaterialEditText edi_current,edi_new,edi_confirm;
    Button btn_edit_password_save;
    SharedPreference sharedPreference;

    public Security_password() {
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {

        View rootview = inflater.inflate(R.layout.fragment_security_password, container, false);

        MainActivity.setToolbarname("Change Password");
        ...
        btn_edit_password_save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                }  else if (edi_new.getText().toString().equalsIgnoreCase("")){
    ...
    private void edit_password(final String regid,final String currentp,final String newp,final String token) {
        if (AppGlobal.isNetwork(Objects.requireNonNull(getActivity()))) {
            ...
            new RestClient(getActivity()).getInstance().get().edit_password(optioMap).enqueue(new Callback<CommanResponse>() {
                @Override
